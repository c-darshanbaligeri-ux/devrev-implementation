...
- **AI agent skill / workflow tool default template**: whenever the user asks to create an "AI agent skill", "agent workflow", "workflow tool", or "agent-callable skill", **always start from `skills/6-workflows/examples/default-ai-agent-skill-template.json`** as the scaffold — never hand-author the four-block wiring (trigger → skill block → HTTP → set output) from scratch. Customize per `skills/6-workflows/references/ai-agent-skill-pattern.md` § "Default starter". This is the shortest path from "user asks" to "importable template"; the four-block structure has enough failure modes (missing `block_step_reference_key`, wrong reference-key format for `$get`, missing `labels: ["skill"]`) that a validated scaffold beats reconstruction every time.
