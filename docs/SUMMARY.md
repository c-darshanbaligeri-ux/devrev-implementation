...
| `.claude/hooks/bootstrap-workspace.sh` | S3 bootstrap hook | Extended with: (1) `.env` gate, (2) workspace dirs creation, (3) repos/ initial clone (background, once, appends per-machine outcomes to `docs/CLONE_RESULTS.local.md` — the gitignored per-machine variant, kept separate from the curated build-time `docs/CLONE_RESULTS.md`), (4) dashboard-sync CLI install via pipx (background, once) |
...
| `.claude/skills/implementation-router/SKILL.md` | Authored new | Master router with routing table (10 skills 0–9 + update-repos + capture-learnings); preconditions check (.env, ping); never reconstruct from memory |
...
| `README.md` | Authored new | Sections: what this is, one-time setup, what self-configures (table), capability map (10 skills 0–9), plug-and-play acceptance, troubleshooting |
...
3. **repos/ clone**: Background once; writes per-repo status to `docs/CLONE_RESULTS.local.md` (gitignored, per-machine — the curated `docs/CLONE_RESULTS.md` is a manually maintained build-time snapshot the hook doesn't touch)
