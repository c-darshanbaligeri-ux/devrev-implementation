...
- **2026-07-19 · Fresh E2E review found two overclaims and one stale design rule in `references/api-cookbook.md`, now corrected in place.** (1) §2 and §5 claimed skill 1's `Custom_Objects_and_Links_API.md` documents `.list` filter syntax, an `id`/`display_id` response-shape breakdown, and a `leaf_only` descriptor option — none of those exist in that file; the pointer text now says what the target actually covers. (2) §4's design rule "a listed stage can't be deleted (mark `is_deprecated: true`)" was stale relative to a correction that had already landed in skill 2's own reference doc and root `CLAUDE.md` — `is_deprecated: true` is rejected everywhere it's tried; there is no working stage-retirement mechanism. (3) Separately, §4 now also reflects a 2026-07-19 finding that stage-diagram attachment to a subtype/custom leaf type DOES work (via `stage_diagram` bare-string or `is_default:true`), correcting an earlier "no working mechanism" conclusion — see skill 2's Field notes for full detail, including the enforcement caveat (works for custom objects, unconfirmed for stock-type subtypes).
- **2026-07-19 · `references/agents.md` line 226 corrected — the read endpoints for agent plans DO exist.** `ai-agents-plans.get`/`ai-agents-plans.list` (hyphen before `plans`) are real, `/internal/`-only, and return `ai_agent_plan` objects including `guidance` runbook text. Only the dotted spelling `ai-agents.plans.*` 404s. This also corrects a claim in the outer `devrev-solution-architect` project's own root `CLAUDE.md` (a different file from this repo's `CLAUDE.md`), which had folded in the now-superseded "do not exist" finding.
