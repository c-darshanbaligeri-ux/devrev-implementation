...
3. **Verify** — `custom-objects.list` to confirm the record exists; `schemas.custom.list` with **`is_custom_leaf_type: true`** to confirm the schema (a bare `.list`, or one filtered by `leaf_type` alone, returns 0 rows for a custom leaf type — see Field notes 2026-08-04).
...
- **2026-07-18 · `schemas.custom.set` behaves like versioned create, not full-replacement update.** Calling `.set` twice with the same `leaf_type` produces TWO fragments (`tenant_fragment/N`, `tenant_fragment/N+1`). The prior version doesn't get replaced; there is no hard-delete of a previous version from the public API (that part still stands — see the 2026-07-26 row below for the *deprecation*, not delete, mechanism that does exist). Test carefully — every `.set` call is an additive commit. Reference doc's "full replacement" language means fields *within* a single fragment, not "supersede the previous fragment". <!-- corrected 2026-07-26: dropped the "...or deprecate..." clause — deprecation via `deprecated: true` is now confirmed working, see below. -->
- **2026-07-23 · RESOLVED by the 2026-07-26 row below — fragment/subtype deprecation via `.set` DOES work, using field name `deprecated`, not `is_deprecated`.** <!-- corrected 2026-07-26: was "Unresolved conflict, needs a live test" claiming the two sources (DevRev docs vs. this repo's 2026-07-18 finding) couldn't both be right — they were both partially right; the input field name was the missing piece. -->
- **2026-07-26 · `schemas.custom.set` deprecation field is `deprecated` (input), NOT `is_deprecated`.** Sending `is_deprecated: true` in a `.set` payload (full field-replay + `type`/`leaf_type`/`subtype` included) returns generic HTTP 400 `bad_request` with no diagnostic field name — easy to misattribute to something else in the payload. Sending `deprecated: true` instead succeeds (HTTP 200 `{"id":...}`), bumps `object_version`, and a follow-up `schemas.custom.get` echoes it back as `is_deprecated: true` (output field name differs from input — same alias-asymmetry already known for `link-types.custom.update`). **Functional effect confirmed real**: after deprecating the `defect` subtype of `raw_test_case` (`custom_type_fragment/5`) this way, `schemas.subtypes.list {"leaf_type":"raw_test_case"}` returned `{"subtypes":[]}`, while the same call against a known-active, non-deprecated subtype (`aibuddytestcase` on `ticket`) correctly listed it — so `subtypes.list` genuinely reflects deprecation status, not just staleness. Not tested: whether `custom-objects.create`/`works.create` with the deprecated subtype's `custom_schema_spec` is now rejected outright (no test record created this session) — `schemas.aggregated.get` with that subtype still resolves its fields when explicitly requested, so deprecation appears to hide-from-listing rather than hard-block; treat "does it block new record creation" as still open.
...
- **2026-07-18 · `schemas.custom.delete` does not exist (route 404).** Custom fragments cannot be hard-deleted via the public API. Combined with the versioned-create behavior above, this makes accidental fragment proliferation the primary risk of running `.set` blindly. For retiring (not deleting) an unwanted subtype/fragment, see the `deprecated: true` mechanism in the 2026-07-26 row above.
...
- **2026-08-04 · `schemas.aggregated.get` returns custom fields under `custom_fields` — there is NO `fields` key at all.** Reading `schema["fields"]` yields nothing and makes every object look like it has zero custom fields, with no error to tip you off. Evidence: `GET schemas.aggregated.get?leaf_type=test_data&is_custom_leaf_type=true` → the schema object's complete key set is `custom_composite_schemas`, `custom_fields`, `description`, `is_custom_leaf_type`, `leaf_type`, `stock_composite_schemas`, `stock_fields`, `title`. On that org's `test_data`, `len(schema.get("fields", []))` → **0** while `len(schema["custom_fields"])` → **46**. Stock fields live in the parallel `stock_fields` key; the merged view is the union of the two, not a single flat list. Any field-count/field-presence check must read `custom_fields` (+ `stock_fields` if stock fields matter) — this is the same silent-empty failure mode as the 2026-07-19 "omit `is_custom_leaf_type=true` → empty schema" row, and the two compound: get either wrong and you get a confident zero.
- **2026-08-04 · `schemas.custom.list` surfaces custom leaf-type fragments ONLY when passed `is_custom_leaf_type: true`; a `leaf_type` filter alone returns zero rows.** The discriminator is `is_custom_leaf_type`, not `leaf_type` — filtering by `leaf_type` by itself silently returns `{"result":[]}` (HTTP 200, no error) even for a leaf type whose fragment demonstrably exists. Evidence, same org, same session: `{}` → 200, 9 rows, all **stock** (`comment`, `conversation`, `ext_comm_channel_instance`, `issue`, `part`, `ticket`); `{"leaf_type":"test_data"}` → 200, **0 rows**; `{"leaf_type":"test_data","is_custom_leaf_type":true}` → 200, **1 row**; `{"is_custom_leaf_type":true}` → 200, **9 rows**, every custom leaf type in the org. So a bare `.list` is not a listing of "all custom fragments" — it is the stock-fragment half only, and you need two calls (with and without the flag) to see everything. Also probed: `{"type":"tenant_fragment"}` → `HTTP 400 invalid_field field_name:"type"` — there is no `type` filter on this endpoint. This makes a bare `.list` unsafe as the "does a fragment already exist for this leaf_type?" precheck that CLAUDE.md's Safety section requires before `schemas.custom.set` — for a custom leaf type it will report "no fragment" and lead you into an accidental versioned-create.
- **2026-08-04 · Deprecating a custom leaf type's tenant fragment is a HARD WRITE BARRIER — it blocks record creation outright. Closes the open question in the 2026-07-26 row above.** That row deprecated a *subtype* fragment, found it hidden from `schemas.subtypes.list`, and left "does it block new record creation" explicitly open. Answer, verified live on the `validation_point` custom leaf type: `schemas.custom.set` with `deprecated: true` (full field replay + `type`/`leaf_type`/`is_custom_leaf_type`/`id_prefix`/`description`) → HTTP 201, and a follow-up `custom-objects.create` against that leaf type → `HTTP 400 {"type":"customization_validation_error","reason":"no tenant fragment found. Make sure the custom object leaf type validation_point has an associated tenant fragment."}`. So deprecation is not merely hide-from-listing: the fragment stops resolving for writes, and the error names a *missing* fragment rather than a deprecated one — misleading if you don't know the type was deprecated on purpose. **This is the closest thing to deleting a custom leaf type that the API offers** (`schemas.custom.delete` still 404s), and it is the right tool for retiring one: records can be deleted, then the fragment deprecated, and nothing can re-create a record afterward. Two sub-findings: (a) `.set` with `deprecated: true` on an existing fragment **updated `tenant_fragment/110` in place** (`object_version` 1 → 2) instead of minting a `/111` — the versioned-create behavior in the 2026-07-18 row does not apply to a deprecation-only `.set`; (b) after deprecation, `schemas.custom.list {"leaf_type":"validation_point","is_custom_leaf_type":true}` returns **0 rows**, so a deprecated fragment is invisible to `.list` but still fetchable by id via `schemas.custom.get`.
- **2026-08-04 · `link-types.custom.list` EXCLUDES deprecated custom link types by default; `{"is_deprecated": true}` is a supported filter that returns only them.** Verified live: a bare `{}` returned 43 types (8 numeric custom + 35 `default-N` stock) with the deprecated `custom_link_type/9` ("AI Buddy Test Case to Validation Point") **absent**; `{"is_deprecated":true}` returned exactly that one; `{"deprecated":true}` also works (same input-alias tolerance as `link-types.custom.update`, per the 2026-07-19 row); `{"include_deprecated":true}` → HTTP 400 `invalid_field`. **This partly re-frames the 2026-07-19 "list-vs-get staleness gap" row**: absence from a bare `.list` is not always staleness — for a deprecated type it is the documented-by-behavior default, and it is permanent, not a propagation delay. Practical consequence: a bare `.list` is a *good* fixture/pre-warm shape precisely because it yields only resolvable types, but it is the WRONG call for auditing whether a retired link type still exists — use the `is_deprecated` filter or `.get` by id. (`.get` by id works fine on both active and deprecated types — HTTP 200 with `is_deprecated` echoed.)
