...
- **2026-07-18 · `custom-objects.create` returns HTTP 403 Forbidden immediately after schema creation — but this is NOT permanent; it resolved on its own within ~20-35 minutes, with no UI action taken.** First call right after `schemas.custom.set` for `regression_test_asset` → HTTP 403 Forbidden (matches the CLAUDE.md rule "zero access by default"). No role was granted via the UI in the meantime (no interactive session touched Settings). A retry roughly 20 minutes later succeeded (HTTP 201, `custom_object/regression_test_asset/1`), and a further retry ~35 minutes after the original 403 also succeeded (`custom_object/regression_test_asset/2`). **Correction to the original claim**: don't assume the fix is strictly UI-only or that the object is permanently invisible — the 403 may be a propagation/indexing delay after schema creation rather than a hard access-control gate requiring manual role grant. If a user hits this 403 immediately after creating a custom object schema, the first troubleshooting step should be **wait and retry** (tens of minutes), not necessarily jump straight to the DevRev Settings UI. Root cause (indexing delay vs. some other default-access process) not confirmed — flagged for a future session to narrow down with precise timing.
