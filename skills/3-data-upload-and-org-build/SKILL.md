...
- **2026-07-31 · `parts.list`'s `parent_part.level` is what caps the depth of the descendant walk — omitting it returns descendants at ANY depth, which is why the 2026-07-20 finding above saw features nested two levels down.** Confirmed against `developer.devrev.ai/beta/api-reference/parts/list` (2026-07-31 fetch): the filter object accepts `parts` (array of DONs, required if any `parent_part.*` field is set) AND `level` (int32, ≥1, "Number of levels to fetch the part hierarchy up to"). So `{"parent_part": {"parts": [<CAP_DON>], "level": 1}}` bounds the walk to direct children only; unset `level` = walk-to-leaves. The 2026-07-20 "matches descendants at any depth" gotcha is really "the default when `level` is omitted." **Practical guidance updated**: for a direct-children-only listing (idempotency check by name, tree drawing), pass `"level": 1` — but for authoritative immediate-parent verification of a single node, `links.list` for `is_part_of` on the specific part is still safer, since `level` filters what `parts.list` *returns* but not what a subsequent per-node parent check needs.
- **2026-07-21 · `parts.update` requires `type` in the payload, not just `id` + the fields being changed.** `{"id":"<don>", "description":"..."}` → HTTP 400 `{"message":"Bad Request","type":"bad_request"}` (no diagnostic field name). Correct shape: `{"id":"<don>", "type":"product", "description":"..."}` → HTTP 200, returns the full updated `part` object. Confirmed live setting the stock `description` field (a `text` field, "Description of the part," present on all part leaf types per `schemas.stock.get`) on 116 parts (product/capability/feature) — every call needed `type` alongside `id`. This is the first documented `parts.update` payload shape in this skill; prior Field notes and the org-build playbook only ever exercised `parts.create`.
