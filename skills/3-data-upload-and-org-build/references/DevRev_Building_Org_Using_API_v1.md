...
- parts.list - confirm the product hierarchy exists. To filter by parent, `parent_part` is an OBJECT, not a bare array: `{"parent_part": {"parts": ["<PARENT_DON>"], "level": <optional int>}}`. Passing a plain array (`{"parent_part": ["<PARENT_DON>"]}`) returns HTTP 400 `unexpected_json_type` (verified live 2026-07-18). **`level` (int, ≥1) is the depth cap** — per `developer.devrev.ai/beta/api-reference/parts/list` (2026-07-31): "Number of levels to fetch the part hierarchy up to." Omit `level` → the API walks descendants at ANY depth (this is what produced the 2026-07-20 finding of features two levels down); set `"level": 1` for direct-children-only. `parts.get`/`parts.list` never return a `parent_part` field on the part record at all. For an authoritative immediate-parent check on a specific node (e.g. idempotency logic before creating a part), use `links.list` with the part as `object`, filtering to `link_type:"is_part_of"` edges where the part is the `source` — the `target` is the true immediate parent.
