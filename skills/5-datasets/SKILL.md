...
- **2026-07-18 · `POST /internal/datasets.list`, `/internal/sources.list`, `/internal/datasets.summary` all return HTTP 404.** Those specific endpoint names don't exist.
- **2026-07-19 · CORRECTS the row above — this is NOT evidence that dataset APIs are plugin-internal-only; those were simply the wrong endpoint names.** The real endpoints, which the `dataset-builder` plugin's own `devrev_api.py` calls internally, work fine over raw REST with the same PAT: `POST /internal/oasis.dataset.list` (200, paginated with `next_cursor`), `.get`, `.create` (201), `.delete` (200 `{}`, requires the full DON `id` — `name`/`dataset_id` both 400 `invalid_field`), and `POST /internal/serengeti.jobs.list` (200). Verified end-to-end: created a test dataset (`claude_fresh_prereq_test`), confirmed via `.get`, deleted via `.delete` with `id`, confirmed gone via a 404 `.get`. Same `/internal/`-only pattern already documented for `ai-agents.*` and `objects.bulk-upgrade` elsewhere in this repo — a hand-rolled REST call IS possible for anyone who wants it, it just needs the right endpoint names, which the plugin itself never had to explain because it already knows them. Still route casual users to `/dataset-builder:*` commands for the validated experience, but don't tell a caller direct REST access is unavailable.
- **2026-07-19 · `oasis.dataset.delete` needs `id` (the full DON), not `name`/`dataset_id`.** Both alternates return `HTTP 400 invalid_field`. The plugin's own `paas-development/SKILL.md` (upstream, `repos/aai-skills/...` — do not edit) shows a `{"name": "my_dataset_name"}` delete example that is factually wrong per live behavior; this Field note is the correction of record since that file isn't ours to fix.
...
- **2026-07-18 · `scripts/check_prerequisites.sh` also flags the Python `requests` library as missing**, alongside the already-known-missing `gcloud`/`bq`/`kubectl` — a from-scratch machine needs `pip install requests` too for scripts that call it directly, even though the PaaS path itself (via the plugin's own HTTP calls) doesn't require it.
- **2026-07-18 · `dashboard-sync dataset list`/`dataset fetch` (shared discovery tooling with skill 4) takes ~6 minutes on an uncached first run** across ~142 datasets, then ~2 seconds once `.cache/dataset_availability.json` exists. Not a hang — budget a long timeout for the first call in this workspace. See skill 4's Field notes for the full detail (same underlying CLI).
