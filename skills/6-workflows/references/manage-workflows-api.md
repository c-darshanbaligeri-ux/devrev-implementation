...
| `workflow-versions.get` | GET | `/internal/workflow-versions.get` | Fetch a version's metadata/`triggers` — **live-verified 2026-07-19: does NOT return the inline step graph**, despite the name suggesting it does. To verify step wiring (`next_steps`, `block_step`, etc.) after building via raw CRUD, call `workflow-steps.get` on each step individually and inspect its `next_steps`/`block_step` fields — that's the only way to confirm the graph, not this endpoint. |
| `workflow-versions.publish` | POST | `/internal/workflow-versions.publish` | Publish a draft version live — **live-verified 2026-07-19: rejected while the workflow is in `Workflow_StatusEnumDraft`** (`HTTP 400 bad_request`, `debug_message` states the enum by name); 8 other plausible names (`workflows.deploy`/`.activate`/`.publish`/`.enable`/`.status.update`, `workflow-versions.deploy`/`.activate`/`.set-active`) all 404. There remains no confirmed API path to move a workflow out of draft. |
