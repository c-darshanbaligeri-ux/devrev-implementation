...
This reference is the detailed spec for the inner workflow JSON referenced from `SKILL.md`'s "Author the template JSON" step. Read this in full whenever you're actually assembling `steps`, ports, or `input_values` — the summary in `SKILL.md` intentionally omits these details so the top-level skill stays short.
...
| **Action ops** (`update_*`, `create_*`, `get_*`, etc.) | **Yes** | No | Need input schema so UI renders field picker. Get schemas from `operations/schemas/<slug>.md` — resolved schemas live in this folder |
...
**Port schema format** — uses the internal schema format with `field_descriptors`, `composite_schemas`, `data_name`, `db_name`, `oasis` metadata. The resolved schemas live in `operations/schemas/<slug>.md` — read those files for the exact port schema objects.
...
- Look up the correct slug from `skills/6-workflows/operations/*.md` files
...
- **Triggers (145):** `skills/6-workflows/operations/triggers.md`
- **Actions (245):** `skills/6-workflows/operations/actions.md`
- **Blockings (2):** `skills/6-workflows/operations/blockings.md`
- **Controls (8):** `skills/6-workflows/operations/controls.md`
...
Study these real-world templates in `skills/6-workflows/examples/` for patterns:
...
|------|-------|---------------------|
| `default-ai-agent-skill-template.json` | 4 | **DEFAULT starter for AI agent skill workflows ONLY** — the four-block shape (ai_agent_skill_trigger -> ai_agent_skill block -> http -> set_ai_agent_skill_output), `labels: ["skill"]`, relies on operation defaults for port schemas. Copy this first for asks like "create an AI agent skill" / "a skill for the agent to invoke", then customize per `ai-agent-skill-pattern.md`. **Do not use for generic (event-triggered or manual) workflows** — those have no default scaffold; pick the closest `working-*.json` below. |
...
11. Input field names match the operation's input schema (check `skills/6-workflows/operations/*.md`)
