# Guide for Updating Feature Flags in archon-policy
...
Skill naming and retrieval order (**FetchObjectContext** vs **HybridSearch** vs NL2SQL) are set in **agent config and instructions**, not in these Rego files. Use the agent creation/improvement workflows for that; use this doc for **runtime** toggles (models, workers, guardrail enablement, etc.).
